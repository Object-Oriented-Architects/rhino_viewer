...
// Composite shader: alpha-over onto framebuffer (with AgX tone mapping + sRGB conversion)
// AgX 톤매핑을 적용하여 금속(MeshStandardMaterial)과 동일한 톤 처리를 보장
// NOTE: toneMappingExposure 1.2는 Canvas onCreated (ring-plusplastic/page.tsx)와 일치해야 함
const compositeFrag = /* glsl */ `
  uniform sampler2D tGems;
  uniform bool toScreen;
  varying vec2 vUv;

  // AgX tone mapping (Three.js tonemapping_pars_fragment.glsl.js에서 복사)
  ...
  vec3 AgXToneMapping(vec3 color) {
    const float toneMappingExposure = 1.2;
  ...
  void main() {
    ...
    if (toScreen) {
      vec3 mapped = AgXToneMapping(gems.rgb);
      gl_FragColor = vec4(linearToSRGB(mapped), gems.a);
    } else {
      gl_FragColor = gems;
    }
  }
